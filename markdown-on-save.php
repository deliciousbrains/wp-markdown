...
class CWS_Markdown {
	...
	public function enqueue() {
		wp_enqueue_script( 'markdown-on-save', plugin_dir_url( __FILE__ ) . '/assets/js/markdown-on-save.js', array( 'jquery' ), '20120426' );
	}
	...
	public function wp_insert_post_data( $data, $postarr ) {
		...
		if ( isset( $postarr['ID'] ) ) {
			...
			$has_changed = isset( $post, $post['post_content'] ) ? $data['post_content'] !== addslashes( $post['post_content'] ) : false;
		...
		$check = ( $nonce ) ? isset( $_POST['cws_using_markdown'] ) : false;
	...
	public function submitbox_actions() {
		...
		echo '<style>#cws-markdown img { vertical-align: bottom;margin-right: 10px; } #cws-markdown a:active { outline: 0 !important }</style><wrap id="cws-markdown" style="display: none"><a href="#" onclick="return false;"><img ' . ( !$markdown ? 'style="display:none" ' : '' ) . 'class="markdown-status markdown-on" src="' . plugin_dir_url( __FILE__ ) . '/assets/img/32x20-solid.png" width="32" height="20" /><img ' . ( $markdown ? 'style="display:none" ' : '' ) . 'class="markdown-status markdown-off" src="' . plugin_dir_url( __FILE__ ) . '/assets/img/32x20.png" width="32" height="20" /></a></wrap><script>document.getElementById("cws-markdown").style.display = "none";</script>';
